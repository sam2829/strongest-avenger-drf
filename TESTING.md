...
| --- | --- | --- |
| Wasn't able to post a video. | When trying to make a post that contained a video, this would raise and error and unable to make the post. | Through research and testing I found that it wasn't working as I tried storing the video's or images in the same field using type "FileField" in the Posts model, I then decided to change this so the images and videos were stored in seperate fields which were "ImageField" and "FileField" respectively. This resolved the issue. |
...
| Post without selecting image or video. | When making a post I was able to make the post without selecting and image or a video. | To resolve this I added code in the serializers.py file to raise a validation error if neither an image or video exists in the post. This prevented the user from posting wihtout selecting and image or video and resolved the issue. There is also extra protection for this in the Posts model so that the model wouldn't accept it. |
| Once models set up correctly couldn't post videos. | When trying to post videos I was unsuccessul. | During researching this I found out that I needed to slect a different storage address for videos in Cloudinary. I did this in the Post model and this resolved the issue.|
