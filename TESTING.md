...
| Logged in user can only view their own report. | If a user is logged in and goes to the reports URL, the user can only see a list of reports that belong to that user. | Logged in as several ddifferent users. | Each time I logged in with a different user, reports on display were the reports of that user who was logged in. | Pass |
...
| User can delete a report. | When a user is logged in and is the owner of a report, they are able to delete their report. If not the owner they are unable to retrieve the report. | Logged in as a user and tried deleteing a report I owned. Then attempted the same under a different user for the same report and when logged out. | When logged in and the owner I was able to retrieve and delete the report. When logged in as not the owner or logged out, I was unable to retrieve the report and presented with a 404 not found. | Pass |
